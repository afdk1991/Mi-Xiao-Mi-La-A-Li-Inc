<template>
  ...
  <div v-if="showSuccessDialog" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg p-8 max-w-md w-full mx-4 text-center">
      ...
      <p class="text-gray-600 mb-4">恭喜您，注册成功！</p>
      <div class="bg-gray-100 rounded-lg p-4 mb-4">
        <p class="text-sm text-gray-500">您的用户ID</p>
        <p class="text-3xl font-bold text-blue-600">{{ registeredUserId }}</p>
      </div>
      <p class="text-sm text-gray-500 mb-6">请牢记您的用户ID，可用于登录</p>
      <button
        @click="goToLogin"
