@echo off
REM ============================================
# laal.top 一键部署脚本 (Windows) - 修正版
# Nginx 服务名与 Docker Compose 服务名已匹配
# ============================================
...
echo 部署配置: 05-部署运维层/mixmlaal-deploy-config/docker-compose.laal.top-fix.yml
echo.
goto end
...
cd "%PROJECT_ROOT%05-部署运维层\mixmlaal-deploy-config"
docker-compose -f docker-compose.laal.top-fix.yml down
echo [OK] 服务已停止
goto end
...
docker-compose -f docker-compose.laal.top-fix.yml up -d
echo [OK] 服务已重启
goto end
...
echo [INFO] 服务状态:
cd "%PROJECT_ROOT%05-部署运维层\mixmlaal-deploy-config"
docker-compose -f docker-compose.laal.top-fix.yml ps
goto end

:logs
echo [INFO] 服务日志 (Ctrl+C退出):
cd "%PROJECT_ROOT%05-部署运维层\mixmlaal-deploy-config"
docker-compose -f docker-compose.laal.top-fix.yml logs -f
goto end
