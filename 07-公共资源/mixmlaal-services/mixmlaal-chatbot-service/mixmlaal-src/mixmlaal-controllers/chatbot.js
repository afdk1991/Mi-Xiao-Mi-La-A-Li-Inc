...
const chatbotController = {
  ...
  ask: async (req, res) => {
    ...
    try {
      const systemPrompt = '你是一个专业的智能客服助手。请用简洁、友好的方式回答用户的问题。如果用户询问订单、配送、退款等业务相关问题，引导用户提供必要的信息（订单号、任务ID等）。';

      const aiResult = await aiService.chat(question, systemPrompt);

      if (!aiResult.success) {
        return res.status(500).json({
          code: 500,
          message: 'AI服务调用失败: ' + aiResult.error,
          data: null
        });
      }
