    ...
    <div v-if="showSuccessModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 overflow-hidden">
        <div class="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-4">
          <h3 class="text-xl font-bold text-white text-center">注册成功</h3>
        </div>
        <div class="px-6 py-8 text-center">
          ...
          <p class="text-gray-800 text-lg mb-4">恭喜您，注册成功！</p>
          <div class="bg-gray-100 rounded-lg p-4 mb-4">
            <p class="text-gray-600 text-sm mb-2">您的用户ID：</p>
            <p class="text-2xl font-bold text-blue-600">{{ registeredUserId }}</p>
          </div>
          <p class="text-gray-500 text-sm mb-6">请牢记您的用户ID，可用于登录</p>
          <button
            @click="goToLogin"
            class="w-full py-2 px-4 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200"
          >
            立即登录
          </button>
        </div>
...
async function handleRegister() {
  ...
  try {
    ...
    if (data.status === 'success') {
      registeredUserId.value = data.data.id;
